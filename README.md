...
- `rate_history.json`: [rate_history.json](./rate_history.json) — historial acumulado de tasas del USD. Cada entrada registra los mismos campos que `rate.json` (`date`, `rate`, `source`, `fetched_at`). El archivo crece con una nueva entrada por día y no se sobreescribe; nunca se eliminan entradas pasadas.